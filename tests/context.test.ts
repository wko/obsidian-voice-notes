...
import { prepareNoteContext, cleanupInput, MAX_CONTEXT_CHARS } from '../src/context';
...
test('context and vocabulary remain separate source fields; only raw transcript is cleaned', async () => {
  ...
  assert.equal(await provider.clean('New words', 'model', 'Rules', { noteContext: 'Existing note', vocabulary: 'Obsidian; Fractals' }), 'Cleaned addition');
});
...
test('a persisted context is not sent when the job did not opt in', async () => {
  ...
  await processJob(job, { transcriber: { async transcribe() { assert.fail('already transcribed'); } }, cleaner: { async clean(_raw, _model, _prompt, context) { assert.equal(context?.noteContext, undefined); return 'Clean'; } }, async save() {}, async append() {} });
});
